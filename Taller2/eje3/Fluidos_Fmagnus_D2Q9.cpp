...
public:
  ...
  void ImponerCampos(double Uventilador,double omega);
...
double LatticeBoltzmann::Fi(double Ux0,double Uy0,double Fx,double Fy,int i){
  ...
  return 3*(1-0.5*Utau)*w[i]*(FdotVi-UdotF+3*UdotVi*FdotVi);
}

void LatticeBoltzmann::Colisione(double gx,double gy){
  int ix,iy,i; double rho0,Ux0,Uy0; double Fx,Fy; int ixc=128, iyc=32, R=8, R2=R*R;
...
void LatticeBoltzmann::ImponerCampos(double Uventilador,double omega){
    ...
    for (iy = 0; iy < Ly; iy++) {
      ...
      if(ix==0){ //ventilador
        ...
        if ((ix-ixc)*(ix-ixc)+(iy-iyc)*(iy-iyc)<=R2) {
          for (i = 0; i < Q; i++) {
            fnew[ix][iy][i]=feq(rho0,-omega*(iy-iyc),omega*(ix-ixc),i);
          }
        }
      }
    }
...
void LatticeBoltzmann::Interpolar(double x,double y,double dAx,double dAy,double &Fx,double &Fy){
  double u,v; int n=(int)x; int m=(int)y; double Deltax, Deltay;
...
int main(void) {
  ...
  int i;
  int t,tmax=1000,dt=1.0;
  double Uventilador=0.1,dphi=2*M_PI/N;
  ...
  double F_magnus, C_m=1.0;


  for (omega = 0; omega < 0.2; omega+=0.01) {
    double F_forzamiento=0.5*C_m*RHOinicial*R*omega*Uventilador;
    //Inicie
    Aire.Inicie(RHOinicial,Uventilador,0);
    //Corra
    for (t = 0; t < tmax; t+=dt) {
      Aire.Colisione(0,F_forzamiento);
      Aire.ImponerCampos(Uventilador,omega);
      Aire.Adveccione();
    ...
    F_magnus=0.5*C_m*RHOinicial*2*R*R*omega*Uventilador;  //Fuerza de magnus en la direccion vertical
    cout<<F_magnus<<"\t"<<sumaFy<<endl;
  }
