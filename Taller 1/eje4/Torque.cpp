...
void Colisionador::CalculeFuerzas(Cuerpo * Pendulo){
  ...
  for(i=0; i<N; i++) Pendulo[i].AdicioneTorque(-Pendulo[i].l*Pendulo[i].m*g*std::sin(Pendulo[i].Theta));
...
int main(void){
  ...
  double t, tdibujo, tmax=0.3*T, tcuadro=T/75, dt=0.0000001;
...
//-------------------------------(theta0, omega0, m0,R0, L0, x00)
 ...
 //-------------------------------Dibujar animacion----------------------------------------------------
   ...
   */
    std::cout << t <<"\t"<< Pendulo[2].GetTau() <<endl;

    
 //-----------------------------------muevase por OMELYAN PEFRL----------------------------------------------
    
    for(i=0; i<N; i++)Pendulo[i].Mueva_Theta(dt,E);
    Newton.CalculeFuerzas(Pendulo);  for(i=0; i<N; i++)Pendulo[i].Mueva_Omega(dt,coeficiente1);
    for(i=0; i<N; i++)Pendulo[i].Mueva_Theta(dt,X);
    Newton.CalculeFuerzas(Pendulo);  for(i=0; i<N; i++)Pendulo[i].Mueva_Omega(dt,L);
