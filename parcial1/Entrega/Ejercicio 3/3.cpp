...
void Cuerpo::Oscile(double t){
  vector3D osc;
  osc.cargue(0,A*std::sin(W*t),0);
  r+=osc;
  //V.cargue(0,A*W*std::cos(W*t),0);
}
void Cuerpo::Pare(double t){
  vector3D osc;
  osc.cargue(0,A*std::sin(W*t),0);
  r-=osc;
  //V.cargue(0,A*W*std::cos(W*t),0);
...
int main(void){
  ...
  int ix,iy, i;
    ...
    double t, tdibujo, tmax=1000, tcuadro=tmax/700, dt=0.0001;
 ...
 // hacer un plot de la altura contra el tiempo
    ...
    Grano[N+1].Oscile(t); //hace que la pared de abajo oscile
